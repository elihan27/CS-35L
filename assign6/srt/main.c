...
void* thread(void *threadnumber)
{
  ...
  const double subsample_dy
    ...
    /* for every pixel */
  for( int px=col; px<width; px+=nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            pixels[px][py][0] = gamma( pixel_color[0] ) * max_color;
            pixels[px][py][1] = gamma( pixel_color[1] ) * max_color;
            pixels[px][py][2] = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
	    for( int i=0; i<3; i++)
	      pixels[px][py][i] = max( min(pixels[px][py][i], 255), 0);
	    // for( int i = 0; i<3; i++)
	    // pixels[px][py][i] = scaled_color[i]; 
	}
...
main( int argc, char **argv )
{
    ...
    numThreads = malloc(nthreads*sizeof(pthread_t));
    ...
    */    
    ...
    for (int i = 0; i!=nthreads; i++)
      {
	thread_id[i]=i;
	err = pthread_create(&numThreads[i], NULL, thread,&thread_id[i]);
	if (err!=0)
	  {
       ...
       for (int i = 0; i!=nthreads; i++)
      {
	err =pthread_join(numThreads[i], NULL);
	if (err!=0)
	  {
